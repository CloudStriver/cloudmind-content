...
type (
	IEsMapper interface {
		Search(ctx context.Context, query []types.Query, fopts *FilterOptions, popts *pagination.PaginationOptions, sorter esp.EsCursor) ([]*Coupon, int64, error)
...
func (m *EsMapper) Search(ctx context.Context, query []types.Query, fopts *FilterOptions, popts *pagination.PaginationOptions, sorter esp.EsCursor) ([]*Coupon, int64, error) {
	...
	coupons := make([]*Coupon, 0, len(hits))
	for i := range hits {
		hit := hits[i]
		coupon := &Coupon{}
		...
		err = mapstructure.Decode(source, coupon)
		if err != nil {
			return nil, 0, err
		}

		oid := hit.Id_
		coupon.ID, err = primitive.ObjectIDFromHex(oid)
		if err != nil {
			return nil, 0, consts.ErrInvalidId
		}
		coupon.Score_ = float64(hit.Score_)
		coupons = append(coupons, coupon)
	}
	// 如果是反向查询，反转数据
	if *popts.Backward {
		for i := 0; i < len(coupons)/2; i++ {
			coupons[i], coupons[len(coupons)-i-1] = coupons[len(coupons)-i-1], coupons[i]
		}
	}
	if len(coupons) > 0 {
		err = p.StoreCursor(ctx, coupons[0], coupons[len(coupons)-1])
		if err != nil {
			return nil, 0, err
		}
	}
